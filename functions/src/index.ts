...
export const sendOnboardingEmail = onDocumentCreated(
  ...
  async (event) => {
    ...
    const audienceId = (interestTag && AUDIENCE_MAP[interestTag as string]) ?
      AUDIENCE_MAP[interestTag as string] : WAAS_LEADS_AUDIENCE_ID;

    {
      try {
        console.log(
          `Adding contact to audience: ${audienceId} ` +
          `for interestTag: ${interestTag}`
        );
        ...
        if (contactError) {
          console.error(
            `Failed to add ${email} to audience ${audienceId}:`,
            contactError
          );
        } else {
          console.log(
            `Successfully added ${email} to audience ${audienceId}. ` +
            `Contact ID: ${contactData?.id}`
          );
        }
      } catch (err) {
        console.error(
          `Unexpected exception adding ${email} to audience ` +
          `${audienceId}:`,
          err
        );
      }
      ...
      try {
        const {data, error} = await resend.emails.send({
          ...
          html: `
            ...
            "max-width: 600px; margin: 0 auto; background-color: #f9f9f9; " +
            `padding: 20px; border-radius: 8px; border-top: 4px solid #a855f7;">
              <h2 style="color: #a855f7; margin-top: 0; ` +
              "text-transform: uppercase; letter-spacing: 1px;\">" +
              `System Audit Initiated</h2>
              <p>Commander ${name || "there"},</p>
              <p>Congratulations on your decision to upgrade your digital ` +
              "footprint. This is the first step toward architecting " +
              `your unfair advantage.</p>
              <p>Our designated AI agents are currently reviewing your ` +
              "architecture and processing your project scope. We are " +
              "preparing tailored insights for our upcoming transmission " +
              "on Day 1 of " +
              `your journey with us.</p>
              <p>While you stand by for the calendar sync, we invite ` +
              "you to review our " +
              "<a href=\"https://waitaminutedigital.com/#solutions\" " +
              "style=\"color: #a855f7; font-weight: bold; " +
              "text-decoration: none;\">" +
              "Selected Masterpieces</a> showcasing high-impact digital " +
              `experiences built for speed, scale, and ROI.</p>
              <br/>
        ...
        if (error) {
          ...
          if (
            ...
            console.warn(
              `Resend 500-level error: ${errorMessage}. ` +
          ...
          console.log(
            "Successfully sent Day 1 onboarding email to " +
            `${email} via Resend. ID: ${data?.id}`
          );
          success = true;
        }
      } catch (err) {
